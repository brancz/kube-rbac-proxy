...
func TestInitTransportWithClientCertAuth(t *testing.T) {
	...
	tlsServer := http.Server{Handler: http.HandlerFunc(testHTTPHandler)}
	go func() {
	...
	resp, err := i.UpstreamTransport.RoundTrip(httpReq)
	if err != nil {
...
func TestKubeRBACProxyInfoSetUpstreamTransport(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "CA set", upstreamCAPath: clientCAPath, expectedRootCA: true},
		{name: "upstream client certs set", upstreamClientCertPath: clientCertPath, upstreamClientKeyPath: clientKeyPath, expectedCerts: true},
		{name: "both set", upstreamCAPath: clientCAPath, upstreamClientCertPath: clientCertPath, upstreamClientKeyPath: clientKeyPath, expectedCerts: true, expectedRootCA: true},
		{name: "cert set/key unset", upstreamClientCertPath: clientCertPath, wantErr: true},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Fatalf("KubeRBACProxyInfo.SetUpstreamTransport() error = %v, wantErr %v", err, tt.wantErr)
			}
			if err != nil {
				return
			}
...
func setupCerts(t *testing.T) (serverCertPath, serverKeyPath, clientCAPath, clientCertPath, clientKeyPath string, err error) {
	...
	if err != nil {
		return "", "", "", "", "", fmt.Errorf("failed to create a new serving cert: %w", err)
	}
	...
	if err != nil {
		return "", "", "", "", "", fmt.Errorf("failed to create a client cert: %v", err)
	}
	...
	return serverCertPath, serverKeyPath, clientCAPath, clientCertPath, clientKeyPath, nil
}
