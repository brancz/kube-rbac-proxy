...
const (
	DefaultCertsURL  = "/certs"
	DefaultConfigURL = "/.well-known/openid-configuration"

	defaultAlgorithm = "RS256"
	defaultAudience  = "test-client-id"
	defaultClientID  = "test-client-id"
	defaultIssuer    = "https://mock-issuer.default.svc.cluster.local:8443"
	defaultKeyID     = "01234567890"
	defaultRole      = "metrics"
	defaultUse       = "sign"
	defaultUsername  = "test-client"
)
...
func CreateOIDCIssuer(issuerURL, mockServerURL string) Action {
	return func(ctx *ScenarioContext) error {
		...
		err := mockClient.RegisterExpectation(mockserver.Expectation{
			Request: mockserver.RequestMatcher{
				Method: http.MethodGet,
				Path:   DefaultConfigURL,
			...
			Response: mockserver.Response{
				Body: map[string]interface{}{
					...
					"jwks_uri":                              fmt.Sprintf("%s%s", issuerURL, DefaultCertsURL),
...
func CreateOIDCToken(client kubernetes.Interface, token *OIDCTokenConfig, mockServerURL string) Action {
	return func(ctx *ScenarioContext) error {
		...
		jwk, jwt, err := createOIDCToken(token.Name, token)
		if err != nil {
		...
		var jwkEntry interface{}
		err = json.Unmarshal(jwk, &jwkEntry)
		if err != nil {
			return fmt.Errorf("failed to unmarshal jwk into object: %w", err)
		}

		// Register an expectation with the mock-server for the /certs endpoint to return the JWK entry
		mockClient := mockserver.NewClientURL(mockServerURL)
		err = mockClient.RegisterExpectation(mockserver.Expectation{
			Request: mockserver.RequestMatcher{
				Method: http.MethodGet,
				Path:   DefaultCertsURL,
				Body: mockserver.BodyMatcher{
					Type:      mockserver.MatchBodyJSON,
					JSON:      nil,
					MatchType: mockserver.TolerantMatch,
		...
		if err != nil {
			return fmt.Errorf("failed to register /certs expectations with mock-issuer: %w", err)
		}
...
func createOIDCToken(name string, token *OIDCTokenConfig) ([]byte, []byte, error) {
	...
	serializedToken, err := builder.Claims(customClaims).CompactSerialize()
	if err != nil {
