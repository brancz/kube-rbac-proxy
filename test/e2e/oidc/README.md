...
test these scenarios a mock-server is used in place of a real OIDC issuer (i.e., authorization server) since the
authorization server is not the subject of these tests. It is more convenient to control the test conditions using a
mock-server than it would be if using an actual authorization server.
